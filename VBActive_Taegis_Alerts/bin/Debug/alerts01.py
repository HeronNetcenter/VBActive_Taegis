...
if FIRST_DAY == '1_a_15':
  ...
  last_day_last_month = datetime(today.year, today.month - 1, 15) if today.month > 1 else datetime(today.year - 1, 12, 15)
  ...
  first_day_last_month = datetime(today.year, today.month - 1, 1) if today.month > 1 else datetime(today.year - 1, 12, 21)
  last_day_last_month = datetime(today.year, today.month, 1) - timedelta(days=1)

# EM CASO DE TESTE DESCOMENTAR ABAIXO
# print(TENANT)
# print("EARLIEST= '" + first_day_last_month.strftime('%Y-%m-%d') + 'T00:00:00.000Z' + "' AND LATEST= '" + last_day_last_month.strftime('%Y-%m-%d') + 'T00:00:00.000Z' + "'")

params = {
    "searchRequestInput": {
        "cql_query": "EARLIEST= '" + first_day_last_month.strftime('%Y-%m-%d') + 'T00:00:00.000Z' + "' AND LATEST= '" + last_day_last_month.strftime('%Y-%m-%d') + 'T00:00:00.000Z' + "'",
        "limit": 10000
    }
}

# =====================================================================================================
# MÓDULO ORIGINAL DE GRAVAÇÃO DO ARQUIVO JSON
# =====================================================================================================
...
arquivo = open('alerts01_' + TENANT_ID + '_' + first_day_last_month.strftime('%Y%m%d') + '.json', 'w')
arquivo.write(json.dumps(result, indent=4))
arquivo.close()
# =====================================================================================================

# =====================================================================================================
# MÓDULO PROPOSTO PELA IA GROK5 PARA GRAVAÇÃO DO ARQUIVO JSON
# =====================================================================================================
r = oauth_client.post(f'{API_ENDPOINT}/graphql', json={
        "query": alertsQuery,
        "variables": params,
...
# Extract the number of alerts (assuming the structure matches the expected JSON)
if 'data' in result and 'alertsServiceSearch' in result['data']:
    ...
    if 'alerts' in alerts_data and 'list' in alerts_data['alerts']:
        all_alerts_count = len(alerts_data['alerts']['list'])
        print(f"Total number of alerts collected: {all_alerts_count}")
    else:
...
# Write the raw API response to the JSON file
arquivo = open('alerts01_' + TENANT_ID + '_' + first_day_last_month.strftime('%Y%m%d') + '.json', 'w')
arquivo.write(json.dumps(result, indent=4))
arquivo.close()