...
# O processo será automatizado pelo programa VB.Net de chamada que calculará a data do dia seguinte 
# a partir da data informada como parâmetro.
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('pDate', help='Data no formato YYYY-MM-DD')
parser.add_argument('pClientName', help='Nome do cliente')
parser.add_argument('pEnvironment', help='Environment')
parser.add_argument('pTenantId', help='Id do Tenant')
parser.add_argument('pClientId', help='Id do cliente')
parser.add_argument('pClientSecret', help='Client secret')

args = parser.parse_args()

# ======================= Client Credentials =======================
# tenant settings
data_dia = datetime.strptime(args.pDate, "%Y-%m-%d").date()
...
# Teste de datas para definir o período do dia de ontem - 19/12/2024
# dia_hoje = date.today()
# dia_ontem = date.today() - timedelta(days=1)
data_ontem = data_dia - timedelta(days=1)
...
with service(access_token=access_token):
    ...
    xquery = f"FROM process EARLIEST='{data_ontem}T00:00:00' AND LATEST='{data_dia}T00:00:00' | aggregate count"
...
# ======================= Query - Resultado Impresso  =======================
    for query_result in result:
        for event in query_result.result.rows:
            print(f"Contagem de eventos no dia anterior - Cliente {client}: {event['row_count']}")
            events_count = event['row_count']

# ======================= Query - Resultado Gravado em Arquivo CSV  =======================
    arquivo = open('events_count_' + tenant_id + '_' + data_ontem.strftime('%Y%m%d') + '_' + 
                   data_ontem.strftime('%Y%m%d') + '.csv', 'w')
    arquivo.write(data_ontem.strftime('%Y%m%d') + ';' + data_ontem.strftime('%Y%m%d') + ';' + str(events_count))
    arquivo.close()
